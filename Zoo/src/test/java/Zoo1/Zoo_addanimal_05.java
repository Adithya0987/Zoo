...
public class Zoo_addanimal_05 extends BaseTest {
	@Test
	...
	Assert.assertEquals(ad.getCageNumber().getAttribute("value"), "7000", "The Entered Data Is Not Macthing");
	Reporter.log("The Entered cage number  is Macthing",true);
	ad.getFeedNumber().sendKeys(ToReadData.fromPropertyFile("Feed"));
	Assert.assertEquals(ad.getFeedNumber().getAttribute("value"), "80000", "The Entered Data Is Not Macthing");
	...
	ad.getEditbutton().click();
	//Reporter.log(ad.getAnimalName().getAttribute("value"),true);
	ad.getAnimalName().clear();
	ad.getAnimalName().sendKeys(ToReadData.fromPropertyFile("Animalchange"));
	//Reporter.log(ad.getAnimalName().getAttribute("value"),true);
	Assert.assertEquals(ad.getAnimalName().getAttribute("value"),"Puppy" , "Updation went Worng");
	Reporter.log("Updation Done ",true);
	ad.getSubmi().click();
	Reporter.log(Delet.getText(),true);
	Delet.accept();
