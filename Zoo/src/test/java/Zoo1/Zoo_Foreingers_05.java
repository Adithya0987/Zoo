...
public class Zoo_Foreingers_05 extends BaseTest {
	@Test
	...
	GenID=GenID.replace("Ticket ID:","");
	GenID=GenID.trim();
	FT.getClick_on_Foreigners_Ticket().click();
	FT.getClick_On_Manage_Ticket().click();
	Assert.assertEquals(GenID, FT.getCompareTicketID().getText(), "wrong ID Gen");
	Reporter.log("Ticket ID Is matching",true);
